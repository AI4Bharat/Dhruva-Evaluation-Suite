...
def calculate(
    ...
    for i, gen_path in enumerate(file_list):
        ...
        fs = gen_fs
        if gen_fs != gt_fs:
            gt_x = librosa.resample(gt_x.astype(np.float), gt_fs, gen_fs,orig_sr=22050, target_sr=16000)
